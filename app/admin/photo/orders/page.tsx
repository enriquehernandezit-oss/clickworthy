            ...
            return (
              <Card key={pkg.id}>
                <div className="flex items-baseline justify-between gap-4">
                  ...
                  <span className="text-xs text-muted">{results.length} photos</span>
                </div>
              ...
              <tbody>
                {selfServe.map((row) => (
                  <tr key={row.id} className="border-b border-line">
                    ...
                    <td className="px-3 py-2 tabular-nums text-text">{row.photoCount}</td>
                    <td className="px-3 py-2 tabular-nums font-medium">{formatPhotoCents(row.totalCents)}</td>
                    <td className="px-3 py-2 tabular-nums text-muted">{fmtDate(row.createdAt)}</td>
                    <td className="px-3 py-2 tabular-nums text-muted">{fmtDate(row.completedAt)}</td>
                    <td className="px-3 py-2">
