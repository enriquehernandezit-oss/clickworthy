...
1. **Nightly sourcing** — `worker/jobs/sourceLeads.ts`. Sweeps a **neighborhood grid** (76 cells, `worker/lib/grid.ts`) with Google Places **Nearby Search** (`rankPreference=DISTANCE`, `fine_dining_restaurant` excluded server-side) across Miami / New York / Chicago / Los Angeles / Nashville / Denver / San Diego. Citywide Text Search is prominence-ranked and only ever returned famous, well-photographed destinations (measured: median 9,554 reviews, zero under 500) — the grid returns every restaurant in each small circle nearest-first, so the modest neighborhood spots we actually serve enter the pool. Every field the filters need rides on the Nearby search call itself, so there's no separate Place Details round-trip. Runs behind two self-correcting gates so it never spends past what the pipeline can use:
   - **Backlog gate** — skips the whole run when unsent inventory already exceeds ~14 days of the daily send cap (`isSourcingBacklogDeep`, `lib/pipelineHealth.ts`). Resumes on its own as the backlog drains.
   - **Per-cell cooldown** — a cell that turns up nothing new (or only known chains) rests for 3 nights, then 7, capped so no cell goes unswept more than a week (`shouldSkipCell`/`nextCellStates`, `worker/lib/grid.ts`; state in the `sourcing_cell_state` setting).
   Hard filters (`worker/lib/filters.ts`: 10–2000 reviews — **20 for a place with an owned, emailable website; 10 for everything else**, since a 10–19-review place with no real website is a phone lead either way — `$`/`$$`, operational; no-website places pass through to the phone segment rather than being dropped) → upsert survivors to `restaurants` (`sourced`) and queue enrichment; filter-failures are recorded as `rejected` so they're never re-processed. The nightly candidate cap (`sourcing_nightly_cap` in Controls, falling back to `WORKER_NIGHTLY_ENRICH_CAP`) is what caps *paid* enrichment spend. Google photos are **never stored** (ToS).
2. **Enrichment** — `worker/jobs/enrichRestaurant.ts`. Fetches the homepage **once**, then: optional hospitality-group check → **photo-fit gates** (`worker/lib/photoFit.ts`) — a free structural read of the site (Gate 1) and, for non-sparse sites, one Claude Vision look at the restaurant's *own* best photo (Gate 2); a restaurant that **already has professional photography** is `rejected` here, before any further spend, but only when both gates agree (a sparse site skips Vision entirely) → email discovery (reusing the homepage) + **NeverBounce** verify → **signature dish** (reused from a website food photo the gate already scored, else scored from Google photos) → priority score. Ends `queued`, `needs_manual_email`, or `rejected`.
3. **Touch 1 — draft, then send (two phases in one job)** — `worker/jobs/sendOutreach.ts`.
   - **Draft phase.** Highest-priority `queued` restaurants (with an email, not held/suppressed) → compose the **approved static template** (EN/ES, merges dish + first name, subject rotates across 3 approved lines) → write an outreach row with `status: 'draft'`. In manual-approval mode only the top **~20/day** are staged (best-first — the `outreach_daily_draft_target` setting, default 20), so the approvals queue is a tight batch of your strongest, gate-filtered leads rather than a 200-deep pile. **Nothing is sent.** The batch waits for your approval in `/admin/photo/outreach`.
   - **Send phase.** Every draft you **approved** sends via Gmail from `mail@clickworthytool.com`, up to the daily cap (ramps 30→50/day, or a manual override in Controls — currently 5/day). Sends are picked by a **fair-share allocator across recipient time zones** (`worker/lib/sendAllocation.ts`), not plain oldest-approval-first — Eastern/Central leads open their 9am–12pm local window before Mountain/Pacific ones do, so a plain sort let them exhaust the whole daily cap before Denver or LA ever got a chance. Records thread ids, flips to `sent`, marks the restaurant `contacted`. Re-checks email/suppressed/held at send time (a draft may sit for days).
   ...
   - **3b. Touch 1.5 bump** — `worker/jobs/sendBumps.ts`. 3 days, no reply → one same-thread bump, ever. The approved copy promises we won't follow up again, and the one-bump guard enforces that. Bumps draw from whatever's left of the daily cap after Touch 1's fair-share reservation (`touch1ReservedToday`) — which is itself bounded by the cap, so a large approved-but-out-of-window Touch 1 pile can no longer starve bumps indefinitely.
4. **Reply loop** — `worker/jobs/pollReplies.ts`. Match inbound replies to their thread; store the reply body + sender either way. An opt-out phrase (`stop`, `unsubscribe`, `remove me`, etc. — no longer the literal uppercase `STOP`; the outbound copy reads "reply and say so" instead, to sound less like a bot) auto-suppresses; anything else routed as an opt-out attempt but not auto-caught still surfaces for you to suppress by hand from the dashboard. **Photo attached** → store it, generate the Revenue Impact Card, create the magic link as **`awaiting_edit`**, and alert you. **Reply with no photo** (e.g. "how much?") → alert you to answer from your own Gmail inbox — it is never silently dropped.
...
- **NeverBounce outage alert** — a depleted credit balance used to fail *silently* (every lead just routed to `needs_manual_email`, indistinguishable from a bad night). `verifyEmail()` now pages within the hour on an insufficient-credit error, and the daily sourcing report prints the live balance.
- **Nightly sourcing report** (`worker/jobs/sourcingReport.ts`) — sourced/verified/photo-scored counts, status split, and the NeverBounce balance, emailed a couple hours after the sourcing cron.
